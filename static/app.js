...
function selectedMode() {
  return document.querySelector('input[name="mode"]:checked')?.value || "Studio";
}
...
function fillProjectForm(project) {
  ...
  const mode = document.querySelector(`input[name="mode"][value="${CSS.escape(project.mode || "Studio")}"]`);
  if (mode) mode.checked = true;
  elements.currentProjectName.textContent = project.name;
  elements.sessionLabel.textContent = `${project.mode || "Studio"} project · ${state.messages.length} saved messages`;
...
function renderConversation() {
  ...
  elements.sessionLabel.textContent = `${state.project?.mode || "Studio"} project · ${state.messages.length} saved messages`;
  elements.conversation.scrollTop = elements.conversation.scrollHeight;
  updateControls();
